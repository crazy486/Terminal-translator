...
- [X] T131 Fix native transcript producer-completion stabilization and session-bound contextual invocation self-exclusion; add deterministic regressions, PS5.1 native stress coverage, and publish the unique manual-acceptance artifact in `src/TerminalTranslator.Windows/`, `src/TerminalTranslator.Cli/`, `tests/`, and `specs/002-on-demand-terminal-assistance/validation/t131-capture-self-exclusion.md`
- [X] T132 Install a product-owned executable below `%LOCALAPPDATA%\TerminalTranslator`, expose a collision-safe `tt` command from the managed PowerShell loader, preserve native exit/redirection/self-exclusion behavior, validate upgrade/idempotency/removal and fresh-shell resolution, and publish a unique acceptance artifact in `src/TerminalTranslator.Windows/`, `scripts/`, `tests/`, and `specs/002-on-demand-terminal-assistance/validation/t132-command-discoverability.md`
- [X] T133 Eliminate the installed PowerShell native transcript race at its console-buffer source; add content-free structural diagnostics, a joined-reader/two-pass bounded completion invariant, native-redirection filtering, 500-cycle provider-free installed-product stress, transition coverage, full regression, and immutable evidence in `src/TerminalTranslator.Windows/`, `src/TerminalTranslator.Cli/`, `scripts/`, `tests/`, and `specs/002-on-demand-terminal-assistance/validation/t133-native-completion-race-20260831.md`
- [X] T134 Restore genuine stdin/stdout/stderr console handles for TTY-sensitive native applications while retaining T133 joined-reader capture for capture-safe native commands; add a permanent TTY regression, targeted installed-product acceptance, one bounded stress run, and immutable evidence in `src/TerminalTranslator.Windows/`, `scripts/`, `tests/`, and `specs/002-on-demand-terminal-assistance/validation/t134-tty-semantics.md`
- [X] T135 Establish the assistance-provider failure boundary, preserve exact network/HTTP/timeout/malformed classifications and content-free opt-in diagnostics, make the transport timeout the single production authority without extending its 10-second bound or adding unproven retries, add deterministic reliability regressions, and record evidence in `src/TerminalTranslator.Core/`, `src/TerminalTranslator.Cli/`, `tests/`, and `specs/002-on-demand-terminal-assistance/validation/t135-provider-reliability.md` — accepted in real Windows PowerShell 5.1 with follow-up artifact SHA256 `75B0A96949C7E285E4194F57CDB8730DA64756C21AD326745DBE3DD2B988AB10`
- [X] T136 Audit the production chat-completion request profile, split malformed-response diagnostics at content-free outer/inner/schema boundaries, run serial controlled DeepSeek request-profile benchmarks, preserve T135's ten-second single timeout authority, and record evidence in `research/provider-profile-benchmark/`, `src/TerminalTranslator.Core/`, `src/TerminalTranslator.Cli/`, `tests/`, and `specs/002-on-demand-terminal-assistance/validation/t136-provider-request-profile.md`
- [X] T137 Correct the production DeepSeek profile by disabling thinking for latency-sensitive translation/assistance and enabling API JSON Output only for structured assistance contracts; prove exact serialized wire shape, preserve T135/T136 defensive parsing and network policy, run Release regression and a small live smoke, and publish a unique human-acceptance artifact with evidence in `src/TerminalTranslator.Cli/`, `tests/`, and `specs/002-on-demand-terminal-assistance/validation/t137-non-thinking-structured-provider-profile.md` — accepted/completed in real Windows Terminal + Windows PowerShell 5.1 with artifact SHA256 `021B8D1EBB68E036BD41F49D32E69787F97EB5472E7AD5163669A51E32400368`
