...
$coordFields = [ordered]@{ X = [int16]; Y = [int16] }
$rectFields = [ordered]@{ Left = [int16]; Top = [int16]; Right = [int16]; Bottom = [int16] }
$coordT = New-StructType 'COORD5' $coordFields
$rectT = New-StructType 'SMALL_RECT5' $rectFields
$infoFields = [ordered]@{ dwSize = $coordT; dwCursorPosition = $coordT; wAttributes = [int16]; srWindow = $rectT; dwMaximumWindowSize = $coordT }
$infoT = New-StructType 'CONSOLE_SCREEN_BUFFER_INFO5' $infoFields

$nativeType = $mod.DefineType('Native5', [System.Reflection.TypeAttributes]::Public -bor [System.Reflection.TypeAttributes]::Sealed)
...
for ($y = 0; $y -lt $rows; $y++) {
    ...
    $readArgs = @($h, $line, [uint32]$cols, [uint32]$y, [uint32]0)
