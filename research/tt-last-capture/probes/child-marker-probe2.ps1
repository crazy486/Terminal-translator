...
$coordT = New-StructType 'COORD11' ([ordered]@{ X = [int16]; Y = [int16] })
$rectT = New-StructType 'SMALL_RECT11' ([ordered]@{ Left = [int16]; Top = [int16]; Right = [int16]; Bottom = [int16] })
$infoT = New-StructType 'CONSOLE_SCREEN_BUFFER_INFO11' ([ordered]@{ dwSize = $coordT; dwCursorPosition = $coordT; wAttributes = [int16]; srWindow = $rectT; dwMaximumWindowSize = $coordT })

$nativeType = $mod.DefineType('Native11', [System.Reflection.TypeAttributes]::Public -bor [System.Reflection.TypeAttributes]::Sealed)
...
# Try several output methods and flush, then read the console buffer.
Write-Output 'CHILD_MARKER_WRITEOUTPUT'
[Console]::WriteLine('CHILD_MARKER_CONSOLEWRITELINE')
[Console]::Out.Flush()
$Host.UI.WriteLine('CHILD_MARKER_HOSTUI')
...
for ($y = 0; $y -lt $rows; $y++) {
    ...
    $joined += $line.ToString(0, $read) + "`n"
}
"Contains WRITEOUTPUT: $($joined.Contains('CHILD_MARKER_WRITEOUTPUT'))"
"Contains CONSOLEWRITELINE: $($joined.Contains('CHILD_MARKER_CONSOLEWRITELINE'))"
"Contains HOSTUI: $($joined.Contains('CHILD_MARKER_HOSTUI'))"
$joined.Trim() -split "`n" | Where-Object { $_.Contains('CHILD_MARKER') } | ForEach-Object { $_.TrimEnd() }
