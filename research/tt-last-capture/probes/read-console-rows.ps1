...
$coordT = New-StructType 'COORD12' ([ordered]@{ X = [int16]; Y = [int16] })
$rectT = New-StructType 'SMALL_RECT12' ([ordered]@{ Left = [int16]; Top = [int16]; Right = [int16]; Bottom = [int16] })
$infoT = New-StructType 'CONSOLE_SCREEN_BUFFER_INFO12' ([ordered]@{ dwSize = $coordT; dwCursorPosition = $coordT; wAttributes = [int16]; srWindow = $rectT; dwMaximumWindowSize = $coordT })

$nativeType = $mod.DefineType('Native12', [System.Reflection.TypeAttributes]::Public -bor [System.Reflection.TypeAttributes]::Sealed)
...
"CONSOLE_INFO: buffer=$($size.X)x$($size.Y) cursor=$($cursor.X),$($cursor.Y) window=($($window.Left),$($window.Top))-($($window.Right),$($window.Bottom))"
...
for ($y = 0; $y -lt $rows; $y++) {
    ...
    $text = $line.ToString(0, $read).TrimEnd()
    if ($text.Length -eq 0) { continue }
    if ($text.Length -gt 100) { $text = $text.Substring(0,100) + '...' }
    Write-Output ('ROW{0:D3}: {1}' -f $y, $text)
}
