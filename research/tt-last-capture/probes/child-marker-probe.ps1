...
$coordT = New-StructType 'COORD10' ([ordered]@{ X = [int16]; Y = [int16] })
$rectT = New-StructType 'SMALL_RECT10' ([ordered]@{ Left = [int16]; Top = [int16]; Right = [int16]; Bottom = [int16] })
$infoT = New-StructType 'CONSOLE_SCREEN_BUFFER_INFO10' ([ordered]@{ dwSize = $coordT; dwCursorPosition = $coordT; wAttributes = [int16]; srWindow = $rectT; dwMaximumWindowSize = $coordT })

$nativeType = $mod.DefineType('Native10', [System.Reflection.TypeAttributes]::Public -bor [System.Reflection.TypeAttributes]::Sealed)
...
Write-Output 'CHILD_MARKER_XYZ_BEFORE_READ'
...
"Contains CHILD_MARKER_XYZ_BEFORE_READ: $($joined.Contains('CHILD_MARKER_XYZ_BEFORE_READ'))"
